...
function getCacheKey(title, artist) {
    return CONFIG.storageKeyPrefix + normalize(title) + "_" + normalize(artist);
}

async function garbageCollectCache() {
    try {
        ...
        for (const [key, value] of Object.entries(allData)) {
            if (key.startsWith(CONFIG.storageKeyPrefix)) {
                const lastTime = value.lastAccessed || value.createdAt || now;
                if (now - lastTime > CONFIG.ttlExpire) {
        ...
        console.warn("[Cache] GC failed:", e);
    }
}
...
function cleanText(text) {
    if (!text) return "";
    return text.replace(/\s*[\(\[-\{\<].*?[\)\]-\}\>].*/, "").replace(/\s*-\s*.*$/, "").replace(/official\s+video|music\s+video|official\s+audio|lyric\s+video|hq|mv|pv/gi, "").trim();
}
function parseLRC(lrcString) {
    ...
    for (const line of lines) {
        ...
        if (match) {
            ...
            if (text) result.push({ time: min * 60 + sec + ms, text: text });
        }
    }
    return result;
}
...
function calculateScore(item, qTitle, qArtist, userLang, targetDuration) {
    ...
    if (targetRegex && targetRegex.test(lyricsSample)) score += 100;
...
async function tryApiSearch(title, artist, album, lang, duration) {
    try {
        const query = `${title} ${artist}`.trim().replace(/\s+/g, '+');
        const url = `${CONFIG.apiSearchBase}?q=${query}`;
        ...
        const scoredCandidates = candidates.map(item => ({ item, score: calculateScore(item, title, artist, shortLang, duration) }));
